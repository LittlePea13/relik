...
from relik.reader.data.relik_reader_coref_data import RelikDataset
from relik.reader.lightning_modules.relik_reader_coref_pl_module import RelikReaderPLModule
...
from relik.reader.utils.strong_matching_eval_doc import ELStrongMatchingCallback
...
def train(cfg: DictConfig) -> None:
    ...
    # with open_dict(cfg):
    #     cfg.data.doc_dataset.special_symbols = special_symbols
    ...
    callbacks = [
        ELStrongMatchingCallback(
            to_absolute_path(cfg.data.val_dataset_path), cfg.data.val_dataset, #skip_first=20
        ),
        ModelCheckpoint(
            "model",
            filename="{epoch}-{val_doc_core_f1:.2f}",
            monitor="val_doc_core_f1",
            mode="max",
        ),
        LearningRateMonitor(),
    ]
    if (
        ...
        print("Shuffling dataset callback added")
        callbacks.append(
    ...
    model.relik_reader_core_model.dataset.tokenizer = train_dataset.tokenizer
